<div x-data="{openAccordion:0, sex:['Men', 'Women', 'Kids']}" @click.away="openAccordion=0" class="">
    <!--    Sex/Gender -->
    <h1>
        ...
        <?php foreach($categories as $category) :?>
            <div class="flex flex-col py-2">
                <div class="flex items-center me-4">
                    <input id="orange-checkbox" type="checkbox" value=""
                           class="w-4 h-4 text-orange-500 bg-gray-100 border-gray-300 rounded focus:ring-orange-500 dark:focus:ring-orange-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                    <label for="orange-checkbox" class="ms-2 text-sm font-medium text-neutral-800 dark:text-gray-300"><?= htmlspecialchars(ucfirst($category['name'] ?? 'Category')) ?></label>
                </div>
            </div>
        <?php endforeach; ?>
    </div>
